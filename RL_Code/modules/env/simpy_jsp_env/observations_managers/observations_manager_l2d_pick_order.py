...
class ObservationManagerL2DPickOrder(ObservationManagerInterface):
    ...
    def reset_state(self, jsp_task, max_n_orders, max_n_machines,
        ...
        self.mask_orders_finished = np.full(shape=self.jsp_n_jobs, fill_value=0, dtype=bool) # True - don't pick the order
        self.mask_orders_waiting_4_machines = np.full(shape=self.jsp_n_jobs, fill_value=0, dtype=bool) # True - don't pick the order
    ...
    def state_update_on_operation_start(self, order, machine, sim_clock, episode_step):
        ...
        self.build_obs_dict['l2d']['mask'] = np.logical_or.reduce((self.mask_orders_waiting_4_machines, self.wip_orders_bool_arr, self.mask_orders_finished))
