...
class ObservationManagerOrderCentricShuffle(ObservationManagerInterface):
    def __init__(self, env_obs_dict, norm_obs, action_mask):
    ...
    def reset_state(self, jsp_task, max_n_orders, max_n_machines,
        ...
        self.wip_orders_bool_arr, self.mask_orders_waiting_4_machines, self.mask_orders_finished = \
            np.repeat(False, self.max_n_orders), \
            np.repeat(False, self.max_n_orders), \
            np.repeat(False, self.max_n_orders)
        ...
        # order part
        self.build_obs_dict_shuffled = {'orders': {}, 'machines': {}, 'rest': self.build_obs_dict['rest']}
        self.indices_oders = np.arange(self.build_obs_dict['orders']['order_number'].shape[0])
        self.rng.shuffle(self.indices_oders)
        # machine part
        self.indices_machines = np.arange(self.build_obs_dict['machines']['machine_numbers'].shape[0])
        self.rng.shuffle(self.indices_machines)
        # shuffle accordingly
        for key, values in self.build_obs_dict['orders'].items():
            self.build_obs_dict_shuffled['orders'][key] = values[self.indices_oders]
        for key, values in self.build_obs_dict['machines'].items():
            self.build_obs_dict_shuffled['machines'][key] = values[self.indices_machines]
        if self.action_mask:
            self.values_action_mask = copy.deepcopy(self.build_obs_dict['mask'])
            self.rng.shuffle(self.values_action_mask)
            self.build_obs_dict_shuffled['mask'] = self.values_action_mask
    ...
    def build_observations(self):
        ...
        for key1, val1 in self.obs_true_keys.items():
            ...
            for key2 in val1:
                build_obs_dict_filt[key1][key2] = self.build_obs_dict_shuffled[key1][key2]
    ...
    def state_update_on_operation_start(self, order, machine, sim_clock, episode_step):
        ...
        if self.action_mask:
            # mask to filter candidates based on available machines
            machines_types = self.build_obs_dict['machines']['op_capabilities_lst']
            machine_states = self.build_obs_dict['machines']['machine_states']
            next_required_machines_per_order = self.build_obs_dict['orders']['next_machine_type_required']
            machines_to_orders_status_mask = np.array(
                list(map(dict(zip(machines_types, machine_states)).get, next_required_machines_per_order)))
            self.mask_orders_waiting_4_machines = machines_to_orders_status_mask == 1
            self.build_obs_dict['mask'] = np.logical_or.reduce(
                (self.mask_orders_waiting_4_machines, self.wip_orders_bool_arr, self.mask_orders_finished))
    ...
    def state_update_on_operation_end(self, order, sim_clock, episode_step):
        ...
        if self.action_mask:
            # a mask to filter candidates based on available machines
            machines_types = self.build_obs_dict['machines']['op_capabilities_lst']
            machine_states = self.build_obs_dict['machines']['machine_states']
            next_required_machines_per_order = self.build_obs_dict['orders']['next_machine_type_required']
            machines_to_orders_status_mask = np.array(list(map(dict(zip(machines_types, machine_states)).get, next_required_machines_per_order)))
            self.mask_orders_waiting_4_machines = machines_to_orders_status_mask == 1
            if order.done:
                self.mask_orders_finished[order.number] = True
            self.build_obs_dict['mask'] = np.logical_or.reduce((self.mask_orders_waiting_4_machines, self.wip_orders_bool_arr, self.mask_orders_finished))
