...
def vectorize_env_stb2(environment, envs_number, rnd_seed=0, env_wrapper='DummyVecEnv', **kwargs):
    """
    ...
    def _init():
        if env_wrapper in ENV_WRAPPERS:
            #envs = ENV_WRAPPERS[env_wrapper]([lambda: env_set_rnd_seed(environment, i, rnd_seed) for i in range(envs_number)])
