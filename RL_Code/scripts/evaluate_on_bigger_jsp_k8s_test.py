...
def get_agents_from_wandb(filt_dicts, project, rand_seed, entity):
    api = wandb.Api()
    ...
    for run in runs:
        for exp_key, filt_dict in filt_dicts.items():
            ...
            if (all(bool_list) is True) & ("eval_res_table" in run.summary):
                if run.config["rand_seed"] == rand_seed:
    ...
    for exp, runs in run_dict_eval.items():
        for run in runs:
            ...
            exp_alias_lst.append(config['exp_alias'])
            for f in fl_list:
...
if __name__ == "__main__":
    rand_seed = 5231 # sys.argv[1]
    run_config_file_name = "test/enhanced_baseline_scalability_6x6x6_2_15x15x15/test_r2_70_makespan_6x6x6_2_15x15x15_900jsp_eps02_shuffle_explr_incent_01_6x6x6_0i_eval_config.yml"
    ...
    jsp_eval_lst = collect_transform_jsp_tasks(jsp_ind_start=run_config_pars['jsp_ind_start_eval'],
                                               ...
                                               read_path=jsp_file_path,
                                               **run_config_pars)

    # train and evaluate
    date_stamp = 'na'
    wandb_project = 'validate_rl4jsp_submission'
    wandb_entity = 'vs162304' #os.environ.get('WANDB_ENTITY')
    ...
    # get related runs
    agent_path_lst, exp_alias_lst, date_lst = get_agents_from_wandb(filt_dicts, wandb_project, rand_seed, wandb_entity)
